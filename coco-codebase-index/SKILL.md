...
- `initialize`：首次建立 `.codebase-index/`。
...
1. 确认项目根目录和忽略规则。
2. 若 `.codebase-index/` 不存在，首次使用必须运行 `initialize`；已有索引先运行 `validate`，过期则 `refresh`。
3. 需求分析查询相似功能、入口、模块和现有测试。
4. 缺陷分析从失败入口执行 `impact`，再用实际代码确认调用链和共享调用方。
5. 代码完成后刷新索引，并在交付报告中记录索引状态。

`query` 和 `impact` 在发现索引缺失时会自动初始化；显式执行 `initialize` 仍推荐用于项目首次接入，以便确认扫描范围和排除规则。`refresh` 在没有旧索引时等价于初始化。

## 安全与限制

- 默认排除 `.git`、依赖、构建产物、缓存、`.codebase-index` 和常见 IDE 目录。
